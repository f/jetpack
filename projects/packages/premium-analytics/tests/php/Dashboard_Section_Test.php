...
class Dashboard_Section_Test extends BaseTestCase {
	...
	public function test_sections_route_reflects_woocommerce_availability() {
		...
		$response = rest_get_server()->dispatch(
			new WP_REST_Request( 'GET', '/jetpack/v4/dashboards/' . DASHBOARD_NAME . '/sections' )
		);
	...
	public function test_retired_layout_write_routes_are_not_registered() {
		...
		$retired_requests = array(
			'PUT section layout'    => array( 'PUT', '/jetpack/v4/dashboards/route_layout_dashboard/sections/analytics/traffic/layout' ),
			'DELETE section layout' => array( 'DELETE', '/jetpack/v4/dashboards/route_layout_dashboard/sections/analytics/traffic/layout' ),
			'DELETE all sections'   => array( 'DELETE', '/jetpack/v4/dashboards/route_layout_dashboard/sections' ),
		);
	...
	public function test_script_data_injects_sections_preload_on_dashboard_page() {
		...
		$path    = '/jetpack/v4/dashboards/' . DASHBOARD_NAME . '/sections';
	...
	public function test_script_data_preload_omits_unavailable_sections() {
		...
		$this->assertSame(
			array( 'traffic', 'insights', 'subscribers' ),
			array_column( $preload[ '/jetpack/v4/dashboards/' . DASHBOARD_NAME . '/sections' ]['body'], 'slug' )
		);
	}
