...
<Canvas of={ BarChartStories.ComparisonSingle } />
...
<Canvas of={ BarChartStories.ComparisonMulti } />
...
- In an interactive legend (`legend.interactive`), each comparison group is a single item labelled by its primary series; clicking it toggles the primary and its previous-period series together. When every series is hidden, the axes are dropped and only the empty state is shown.
