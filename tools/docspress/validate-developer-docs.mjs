...
const docspressHeroPattern = /<!-- wp:docspress\/hero (\{.*\}) \/-->/g;
const docspressHeroMatches = [ ...homepage.matchAll( docspressHeroPattern ) ];
...
if ( carouselMatches.length === 1 ) {
	...
	const customHeroIndex = homepage.indexOf( 'className":"jp-home-hero"' );
	const docspressHero = docspressHeroMatches[ 0 ];
	const docspressHeroIndex = docspressHero?.index ?? -1;
	check(
		carouselIndex > Math.max( customHeroIndex, docspressHeroIndex ),
		'homepage: carousel must follow the hero'
	);
	check( carouselIndex < startIndex, 'homepage: carousel must precede the #start section' );
	const contentBeforeCarousel = homepage.slice( 0, carouselIndex ).trimEnd();
	check(
		contentBeforeCarousel.endsWith( '<!-- /wp:group -->' ) ||
			( docspressHero && contentBeforeCarousel.endsWith( docspressHero[ 0 ] ) ),
